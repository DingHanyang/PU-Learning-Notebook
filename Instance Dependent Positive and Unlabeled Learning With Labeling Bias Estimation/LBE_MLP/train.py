...
def pretrain(x_train, q_train, model_pyx, model_eta, lr, wd):
    ...
    for iter_stp in range(100):
        ...
        loss = -(q_train * (1 - porportion_labeled) * p.log() + porportion_labeled * (1 - q_train) * (
                    1 - p).log()).mean()
...
def em_train(x_train, q_train, x_test, y_test, lr, wd):
    ...
    model_pyx = MLP(dim_fea)
    model_eta = MLP(dim_fea)
    ...
    for e_step in range(100):
        ...
        for m_step in range(100):
            ...
            loss2 = -(q1 * ((((1 - eta) ** (1 - q_train[indies])) * (
                        eta ** q_train[indies])).squeeze() + 1e-5).log()).mean()
            loss = loss1 + loss2
...
if __name__ == "__main__":
    ...
    notes: 
    syn的参数 
    type: 策略 3为正常随机打乱策略，1 和 2分别为文章中的策略1、2
    noise_rate: 这个参数用来控制噪声程度，其实就是不同计算方式的Pi
    ...
    for i in range(5):
        ...
        accu.append(temp)
    # -------------------------------------------------------------------- #
    print('accu: %.4f std: %.4f | %.4f %.4f %.4f %.4f %.4f' % (
    t.Tensor(accu).mean().item(), t.Tensor(accu).std().item(), accu[0], accu[1], accu[2], accu[3], accu[4]))
